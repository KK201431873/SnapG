...
def get_csv_lines(seg_data_list: list[SegmentationData], 
    ...
    # get axon metrics
    g_ratios: list[float] = []
    for _, contour_data_list, _ in data_lists:
        g_ratios += [c.g_ratio for c in contour_data_list]
    ...
    csv_lines.append(f"Mean G-ratio,{g_ratio_mean:.4f}\n")
    csv_lines.append(f"G-ratio Stdev,{g_ratio_stdev:.4f}\n")
    csv_lines.append(f"G-ratio SE,{g_ratio_se:.4f}\n")
    csv_lines.append(f"\n")
